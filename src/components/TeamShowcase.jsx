...
const TeamShowcase = () => {
  ...
  const developers = [
    { name: 'Mr Ebad', role: 'Full Stack Developer', avatar: '/user.png' },
    { name: 'Mr Risheb', role: 'Frontend Developer', avatar: '/user.png' },
    { name: 'Mr Abner', role: 'Backend Developer', avatar: '/user.png' },
    { name: 'Mr Shubham', role: 'DevOps Engineer', avatar: '/user.png' }
  ];
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentTeam.map((member, index) => (
            <motion.div
              key={`${activeTab}-${index}`}
              className="group relative bg-gradient-to-br from-gray-800/30 to-gray-900/20 border border-gray-700/50 rounded-2xl p-6 backdrop-blur-sm hover:border-green-500/50 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-green-900/20"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="relative z-10 text-center">
                ...
                <div className="relative mb-6">
                  <motion.div 
                    className="w-24 h-24 mx-auto rounded-full overflow-hidden border-2 border-gray-700/50 group-hover:border-green-500/50 transition-colors duration-300"
                ...
                {/* Member Info */}
                <motion.h3 
                  className="text-lg font-bold text-white mb-2 group-hover:text-green-300 transition-colors"
                  whileHover={{ scale: 1.05 }}
                >
                  {member.name}
                </motion.h3>
